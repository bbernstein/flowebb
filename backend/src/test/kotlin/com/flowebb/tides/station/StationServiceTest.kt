...
class StationServiceTest : DynamoTestBase() {
    ...
    @Test
    fun `findNearestStations tries all sources when preferred source fails`() =
        runBlocking {
    ...
    @Test
    fun `findNearestStations returns stations from preferred source when available`() =
        runBlocking {
            coEvery {
                noaaFinder.findNearestStations(any(), any(), any())
            } returns listOf(mockStation)
    ...
    @Test
    fun `getStation returns station from first available source`() =
        runBlocking {
            coEvery {
