...
class TidePredictionCacheTest : DynamoTestBase() {
    ...
    @Test
    fun `getPredictions returns cached data when valid`() = runTest {
        val now = System.currentTimeMillis()
        val date = LocalDate.of(2024, 3, 20)
        val dateStr = date.format(DateTimeFormatter.ISO_DATE)
        val record = TidePredictionRecord(
            stationId = "TEST1",
            date = dateStr,
            stationType = "R",
    ...
    @Test
    fun `getPredictions returns null for expired cache`() = runTest {
    ...
    @Test
    fun `savePredictionsBatch handles multiple records correctly`() = runTest {
        val now = System.currentTimeMillis()
        val records = listOf(
